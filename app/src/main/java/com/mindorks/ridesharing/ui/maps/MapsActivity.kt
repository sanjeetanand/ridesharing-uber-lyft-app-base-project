...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback, MapsView {
        ...
        if (requestCode == PICKUP_REQUEST_CODE || requestCode == DROP_REQUEST_CODE) {
            when (resultCode) {
                Activity.RESULT_OK -> {
                    ...
                    Log.d("MapAct", "Place: " + place.name + ", " + place.id + ", " + place.latLng)
                    when (requestCode) {
                        PICKUP_REQUEST_CODE -> {
                ...
                AutocompleteActivity.RESULT_ERROR -> {
                    ...
                    //Log.d("MapActivity", status.statusMessage!!)
                }
                Activity.RESULT_CANCELED -> {
                    //Log.d("MapActivity", "Place Selection Canceled")
                }
            }
        }
    }
            ...
            PermissionUtils.isAccessFineLocationGranted(this) -> {
                when {
                    PermissionUtils.isLocationEnabled(this) -> {
                        ...
                        PermissionUtils.showGPSNoteEnabled(this)
                    }
                }
            }
            else -> {
                PermissionUtils.reqAccessFineLocationPermission(this, LOCATION_PERMISSION_REQUEST_CODE)
            }
        }
    }

    private fun setUpCurrentLocationAsPickUp(){
    ...
    private fun setUpLocationListener(){
        ...
        //for getting current location update after every 2 sec
        ...
        when(requestCode) {
            LOCATION_PERMISSION_REQUEST_CODE -> {
                if(grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                    ...
                    Toast.makeText(this,getString(R.string.location_not_granted), Toast.LENGTH_SHORT)
                }
            }
        }
    }
